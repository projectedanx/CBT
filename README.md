...
*   **Meta-Cognitive Reflexive Ecosystem (MCRE):** Replaces verbal Chain-of-Thought with continuous latent steering via soft tokens and Differentiable Cache Augmentation, shifting corrections into the high-dimensional geometric landscape.
...
## 14. Agentic TDD & Cybernetic Integration (Phase 13 Integration)
To ensure the output generated by autonomous agents is deterministically sound, this project enforces strict Agentic Test-Driven Development (TDD) loops.
*   **The Programmatic Oracle:** Transforms subjective natural language instructions into verifiable mathematical feedback loops (Red Phase -> Green Phase -> Refactor Gate).
*   **Role Segregation:** Decouples the Test Architect (creates read-only tests) from the Implementer Agent (mutates source code) to prevent the Sycophantic Mocking vulnerability.
*   **Zero-Trust Sandboxing:** Enforces test execution within the `gemini-cli-sandbox` Docker container without network privileges to prevent arbitrary codebase escapes.
*   **Adaptive Escape Hatch:** Prevents the Token "Doom Loop" by enforcing a hard iteration cutoff ($N=10$) for non-convergent test errors.
For more details, consult `docs/adr/22-agentic-tdd-cybernetic-mechanics.md` and the blueprints in the `research/` directory.
